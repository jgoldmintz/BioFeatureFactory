 # NetPhos Pipeline
  ...
  High-throughput kinase-site prediction for WT and mutant protein FASTAs using NetPhos 3.1/APE. The pipeline auto-detects native Linux installs but falls back to the shared NetNGlyc Docker image on macOS/Windows or when
  ...
  | Component | Notes |
  |-----------|-------|
  | NetPhos 3.1 + APE | Native use (Linux only): install NetPhos, ensure `tcsh` exists, and point to the binary via `--native-ape-path` or `NETPHOS_APE_PATH/NETPHOS_HOME`. |
  | Docker Desktop | Required on macOS/Windows or whenever native NetPhos isn't configured; the pipeline reuses the `biofeaturefactory:latest` container. |
...
```
  ...
  | Option | Purpose |
  |--------|---------|
  | --native-ape-path /opt/netphos/ape-1.0/ape | Explicit binary path (Linux). |
  | --force-native | Fail if the native binary cannot run; no Docker fallback. |
  | --force-docker | Skip native checks even if a binary exists. |
  | Env vars | NETPHOS_APE_PATH, NETPHOS_HOME, NETPHOS_ALLOW_NATIVE=1 influence detection. |
  ...
  | Symptom | Resolution |
  |---------|------------|
  | tcsh: Command not found | Install tcsh (native mode) or ensure Docker image is built. |
  | Docker timeout | Increase --timeout, verify docker ps works, or use native mode if available. |
  | “No mapping file found” | Confirm --mapping-dir contains files named like *{GENE}*.csv. |
  | Native mode ignored | Ensure you’re on Linux and the binary is executable, or pass --force-native. |
