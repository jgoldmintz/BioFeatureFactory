...
High-throughput protein structure prediction for WT and mutant sequences using NetSurfP-3.0. Predicts surface accessibility, secondary structure, disorder regions, and binding sites. Supports both Docker and native execution modes.
...
| Docker Desktop | Required on macOS/Windows or when native NetSurfP isn't configured. |
...
```bash
# The unified biofeaturefactory:latest image includes NetSurfP-3.0 support
cd ../docker
# Build the unified software suite image (requires NetSurfP-3.0 installation)
# Configure NetSurfP-3.0 files in docker/ directory before building
./build-container.sh
```

The unified `biofeaturefactory:latest` image contains NetSurfP-3.0 along with NetNGlyc, NetPhos, NetMHC, and other tools.

### Option B: Native Installation (Linux/macOS with PyTorch)
...
- `--force-native`: Force native execution
...
| Symptom | Resolution |
|---------|------------|
| `NetSurfP not found` | Install NetSurfP-3.0 or build Docker image |
| `PyTorch/ESM not found` | Install PyTorch and download ESM model weights for native mode |
| `Docker timeout` | Increase `--timeout` (NetSurfP can be slow for long sequences) |
...
| Native mode ignored | Ensure Python 3.7+ with PyTorch installed, or use `--force-native` |
