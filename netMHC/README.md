...
High-throughput MHC binding prediction for WT and mutant protein sequences using NetMHCpan 4.1 or other NetMHC tools. The pipeline supports both Docker and native execution modes.
...
| Component | Notes |
|-----------|-------|
| NetMHCpan 4.1 (or NetMHC/NetMHCII) | Either build a Docker image or install natively (Linux only). |
| Docker Desktop | Required on macOS/Windows or when native NetMHC isn't configured. |
...
```bash
# The unified biofeaturefactory:latest image includes NetMHC 4.0
cd ../docker
# Build the unified software suite image
# NetMHC-4.0 is automatically copied from ../../software/netMHC-4.0
./build-container.sh
```

The unified `biofeaturefactory:latest` image contains NetMHC-4.0 along with NetNGlyc and other tools.

**Note**: NetMHC-4.0 must already be installed in `/Volumes/990pro/disease_associated_synonymous_mutations/software/netMHC-4.0/` before building.

### Option B: Native Installation (Linux only)
...
- `--force-native`: Force native execution (Linux only)
- `--force-docker`: Force Docker execution
- `--docker-image NAME`: Docker image name (default: biofeaturefactory:latest)
...
**Note**: Very small batch sizes increase overhead from repeated Docker launches or NetMHC startup time.
...
| Symptom | Resolution |
|---------|------------|
| `NetMHC not found` | Install NetMHCpan or build Docker image. On Linux, ensure binary is executable. |
| `Docker timeout` | Increase `--timeout` or use `--force-native` on Linux |
...
| Native mode ignored | Ensure Linux host and executable permissions, or use `--force-native` |
...
- Configured for linux/386 platform (works on Apple Silicon via emulation)
- Automatic platform detection and command construction
...
Expected output:
```
Building BioFeatureFactory Software Suite Docker container...
