...
| Component            | Notes                                                                                                                                                         |
|----------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------|
| NetNGlyc 1.0         | Docker image (`docker/build-container.sh`) or native binary (Linux: `--native-netnglyc-bin` or `NETNGLYC_PATH` / `NETNGLYC_HOME` env var). |
...
| Docker not installed                                | Use `--native-netnglyc-bin /path/to/netNglyc` (Linux only), or set `NETNGLYC_PATH` or `NETNGLYC_HOME` environment variable. Use `--force-native` to require native execution and fail if binary unavailable. |
