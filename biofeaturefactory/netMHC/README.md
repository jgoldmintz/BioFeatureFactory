...
| Python $\geq$3.9 | Uses shared utilities from `../utils/`. |
...
python netmhc-pipeline.py input/ results/ --mode full-pipeline

# Larger batches for better performance on shorter proteins
python netmhc-pipeline.py input/ results/ --batch-size 500

# Disable batching entirely (process full sequences)
python netmhc-pipeline.py input/ results/ --batch-size 0
```
