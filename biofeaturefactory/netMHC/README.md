...
| Component | Notes |
|-----------|-------|
| NetMHCpan 4.1 (or NetMHC/NetMHCII) | Download from [DTU Health Tech](https://services.healthtech.dtu.dk/services/NetMHCpan-4.1/) (academic license). Requires Linux. Set path via `--native-netmhc-path` or `NETMHCPAN_PATH`/`NETMHC_PATH`/`NETMHC_HOME` env var. |
