...
| Component            | Notes                                                                                                                                                         |
|----------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------|
| NetNGlyc 1.0         | Download from [DTU Health Tech](https://services.healthtech.dtu.dk/services/NetNGlyc-1.0/) (academic license). Set path via `--native-netnglyc-bin` or `NETNGLYC_PATH` / `NETNGLYC_HOME` env var. |
| SignalP 6.0          | Download the **fast** version from [DTU Health Tech](https://services.healthtech.dtu.dk/services/SignalP-6.0/) (academic license). Must be on `PATH`. Requires `numpy<2`. Cache defaults to `~/.signalp6_cache`, but `--cache-dir` relocates it. See section2.1 for NetNGlyc integration. |
| Python $\geq$3.9          | Uses the shared utilities under `../utils`.                                                               |
| Mapping CSVs         | Per-gene NT$\rightarrow$AA mappings (`mutant`, `aamutant`). Generated by exon-aware pipeline.                                |
...
1. **Edit your NetNGlyc installation's `netNglyc` tcsh script** (typically in the NetNGlyc installation directory):

```tcsh
# Replace the existing SIGNALP line with:
setenv SIGNALP /path/to/BioFeatureFactory/biofeaturefactory/netNglyc/bin/signalp6_adapter
```

2. **How it works:**
   - The Python pipeline runs SignalP 6 and stores results in a temp directory
   - It sets `SIGNALP6_RESULTS_DIR` env var pointing to that directory
   - When NetNGlyc's tcsh script calls `$SIGNALP`, the adapter reads the pre-computed SignalP 6 results
   - The adapter outputs the legacy 14-column format that NetNGlyc expects

3. **If `$SIGNALP` is not configured:** NetNGlyc runs without signal peptide context (still functional, but less accurate for secreted proteins).

---

### Full pipeline (WT input $\rightarrow$ TSV ensemble)
...
python netnglyc_pipeline.py \
    ...
    --keep-intermediates
# Writes per gene: results/{GENE}/NetNglyc/{GENE}.{tsv,events.tsv,sites.tsv}
```
...
{output}/
  {GENE}/
    NetNglyc/
      {GENE}.tsv          -- per-mutation summary (WT vs MUT deltas, SignalP context, QC flags)
      {GENE}.events.tsv   -- per-position deltas classified as gained/lost/strengthened/etc.
      {GENE}.sites.tsv    -- raw WT and MUT predictions with motif encodings and SignalP
```
...
| `frac_effect_post_cleavage`                                                          | Fraction of total $\left\lvert \Delta \right\rvert$ downstream of SignalP cleavage (see section4). | fraction (0-1)    |
