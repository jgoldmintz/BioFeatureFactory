...
| SignalP 6.0          | Download the **fast** version from [DTU Health Tech](https://services.healthtech.dtu.dk/services/SignalP-6.0/) (academic license). Must be on `PATH`. Requires `numpy<2`. Cache defaults to `~/.signalp6_cache`, but `--cache-dir` relocates it. See §2.1 for NetNGlyc integration. |
| Python ≥3.9          | Uses the shared utilities under `../utils`.                                                               |
